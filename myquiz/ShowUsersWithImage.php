...
while($row=mysqli_fetch_array($ema, MYSQLI_ASSOC)){
	echo '<tr><td>'.$row['First Name'].'</td><td>'.$row['Last Names'].'</td><td>'.$row['e-Mail'].'</td><td>'.$row['Phone'].'</td><td>'.$row['Department'].'</td><td>'.$row['Tech'].'</td><td><img src="data:image/jpeg;base64,'.base64_encode( $row['Image'] ).'" width="100"/></td></tr>';
}
